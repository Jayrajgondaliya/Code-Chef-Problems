...
public class COOMILK{
	public static void main(String args[]){
		...
		while(t-->0){
            ...
            boolean lastOneCookie = false;

            String ans = "YES";
            if(n == 1 && str[0].equals("cookie")){
                ans = "NO";
            }else{
            for(int i = 0; i < n; i++){
                if(lastOneCookie && !str[i].equals("milk")){
                    ans = "NO";
                    break;
                }
                if(str[i].equals("cookie")) lastOneCookie = true;
                else lastOneCookie = false;
            }
            if(lastOneCookie) ans = "NO";
        }
            answer.add(ans);
		}
