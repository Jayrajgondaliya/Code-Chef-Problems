	...
	public static void main(String args[]){
		Scanner sc = new Scanner(System.in);
				...
				while(t-->0){
					int n = sc.nextInt();
					sc.nextLine();
					String timeHave[] = sc.nextLine().split(" ");
					String timeNeed[] = sc.nextLine().split(" ");
					long endedSlot = 0L;
					long currentSlot = 0L, time = 0L;
					int ans=0;
					for(int i = 0; i < n; i++){
						currentSlot = Long.parseLong(timeHave[i]);
						time = Long.parseLong(timeNeed[i]);
						if((currentSlot - endedSlot) >= time){
							ans++;
						}
