...
public class MSNSADM1{
	public static void main(String args[]){
		...
		while(t-->0){
            ...
            List<Integer> aList = IntStream.range(0, n).mapToObj(e -> Integer.parseInt(aStr[e])).collect(Collectors.toList());
            List<Integer> bList = IntStream.range(0, n).mapToObj(e -> Integer.parseInt(bStr[e])).collect(Collectors.toList());
            List<Integer> ans = new ArrayList<Integer>();
            for(int i = 0; i < n; i++){
                ans.add(aList.get(i) * 20 - bList.get(i) * 10);
            }
