...
public class COUPSYS{
	public static void main(String args[]){
		...
		while(t-->0){
            ...
            int[] cities = {Integer.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE};
            int[] discounts = new int[3];
            for(int i = 0; i < n; i++){
                city = sc.nextInt();
                level = sc.nextInt();
                discount = sc.nextInt();
                if(discount >= discounts[level - 1]){
                    if(discount == discounts[level - 1]){
                        if(city < cities[level - 1]){
                            cities[level - 1] = city;
                        }
                    }else{
                        cities[level - 1] = city;
                    }
                    discounts[level - 1] = discount;
                }
            }
            for(int i = 0; i < 3; i++){
                System.out.println(discounts[i]+" "+cities[i]);
            }
        }
	}
}
