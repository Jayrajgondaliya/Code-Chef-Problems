...
public class MEET{
	public static void main(String args[]){
		...
		while(t-->0){
            ...
            Time meetingTime = new Time(p[0], p[1]);
            int n = sc.nextInt();
            sc.nextLine();
            ...
            for(int i = 0; i< n; i++){
                ...
                start = new Time(temp[0], temp[1]);
                end = new Time(temp[2], temp[3]);
                if(meetingTime.compareTo(start) >= 0 && meetingTime.compareTo(end) <= 0){
                    sb.append("1");
                }else{
                    sb.append("0");
                }
...
class Time implements Comparable<Time>{
    ...
    Time(String time, String s){
        ...
        this.hh = Integer.valueOf(temp[0]);
        this.mm = Integer.valueOf(temp[1]);
        this.AMPM = s;
        if(this.AMPM.equals("AM") && this.hh == 12) hh = 0;
        else if(this.AMPM.equals("PM") && this.hh != 12) hh += 12;
    }

    @Override
    public int compareTo(Time t){
            if(this.hh > t.hh) return 1;
            else if(this.hh < t.hh) return -1;
            else{
                if(this.mm > t.mm) return 1;
                else if(this.mm < t.mm) return -1;
                else return 0;
            }
    }
